  ...
  <div class="p-6 max-w-xl mx-auto">
    ...
    <template v-if="variant === 'assignment'">
      ...
      <div class="mb-4">
        ...
        <InputText v-model="assignmentForm.title" class="w-full rounded-input" placeholder="Type assignment title here" />
      </div>

      <div class="flex gap-4 mb-4">
        <div class="flex-1">
          <label class="block mb-2">Assignment Due Date</label>
          <Calendar v-model="assignmentForm.dueDate" placeholder="Select a Date" class="w-full rounded-input button-clr" showIcon />
        </div>
        <div class="flex-1">
          <label class="block mb-2">Set Attempt Count</label>
          <div class="flex items-center gap-2">
            <Button icon="pi pi-minus" class="button-clr" @click="decreaseAttempts" :disabled="assignmentForm.attempts <= 1" />
            <span>Attempts {{ assignmentForm.unlimitedAttempts ? '∞' : assignmentForm.attempts }}</span>
            <Button icon="pi pi-plus" class="button-clr" @click="increaseAttempts" :disabled="assignmentForm.unlimitedAttempts" />
      ...
      <div class="mb-4">
        ...
        <Dropdown v-model="assignmentForm.format" :options="formats" optionLabel="label" optionValue="value" placeholder="Select a Format" class="w-full rounded-input" />
      </div>
      ...
      <div class="mb-4">
        ...
        <InputText v-model="assignmentForm.detailsHeader" class="w-full rounded-input" placeholder="Type assignment title here" />
      </div>

      <div class="mb-6">
        <label class="block mb-2">Assignment Details Description</label>
        <Textarea v-model="assignmentForm.description" class="w-full rounded-input" rows="6" placeholder="Type assignment description here" />
      </div>
    ...
    <template v-else-if="variant === 'module'">
      ...
      <div class="mb-4">
        ...
        <InputText v-model="moduleForm.title" class="w-full rounded-input" placeholder="Type Module title here" />
      </div>
      ...
      <div class="mb-4">
        ...
        <InputText v-model="moduleForm.pageName" class="w-full rounded-input" placeholder="Type Module Page Name here" />
      </div>

      <div class="mb-4">
        <label class="block mb-2">Module Page Description</label>
        <Textarea v-model="moduleForm.description" class="w-full rounded-input" rows="5" placeholder="Type Module Page Name here" />
      </div>

      <div class="mb-4">
        <label class="block mb-2">Module Page Slides</label>
        <FileUpload mode="basic" class="w-full rounded-input" name="slides" chooseLabel="Upload Page Slides" @select="e => moduleForm.slides = e.files" />
      </div>

      <div class="mb-4">
        <label class="block mb-2">Add Additional Resources</label>
        <Textarea v-model="moduleForm.resources" class="w-full rounded-input" rows="5" placeholder="Type Additional Module Page Resources here" />
      </div>

      <div class="mb-6">
        <label class="block mb-2">Embed Video/Content</label>
        <InputText v-model="moduleForm.embedCode" class="w-full rounded-input" placeholder="Paste Embed code here" />
      </div>
