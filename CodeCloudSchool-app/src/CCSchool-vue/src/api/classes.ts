...
export const ClassesService = {
  ...
  async getClassesByLecturerId(userId: number): Promise<Class[]> {
    const response = await api.get(`classes/byLecturer/${userId}`)

    if (response.status !== 200) {
      throw new Error(`Failed to fetch classes for student ${userId}: ${response.statusText}`)
    }
